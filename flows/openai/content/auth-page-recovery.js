(function authPageRecoveryModule(root, factory) {
  ...
  function createAuthPageRecovery(deps = {}) {
    ...
    function getAuthTimeoutErrorPageState(options = {}) {
      ...
      const emailInUseBlocked = /email_(?:already_)?in_use/i.test(text);
    ...
    async function recoverAuthRetryPage(options = {}) {
      ...
      while (clickCount < maxClickAttempts) {
        ...
        if (retryState.emailInUseBlocked) {
          throw new Error(
            'STEP8_EMAIL_IN_USE::email_already_in_use on auth retry page; choose a different email.'
          );
        }
      ...
      if (finalRetryState.emailInUseBlocked) {
        throw new Error(
          'STEP8_EMAIL_IN_USE::email_already_in_use on auth retry page; choose a different email.'
        );
      }

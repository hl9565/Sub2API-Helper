...
const SIGNUP_EMAIL_EXISTS_PATTERN = /与此电子邮件地址相关联的帐户已存在|该邮箱地址已有相关联账户|この(?:メールアドレス|メール|電子メール)(?:に関連付けられた)?アカウントは(?:既に|すでに)存在|メールアドレス.*(?:既に|すでに)存在|account\s+associated\s+with\s+this\s+email\s+address\s+already\s+exists|email\s+address.*already\s+exists/i;
const EMAIL_IN_USE_ERROR_CODE_PATTERN = /email_(?:already_)?in_use/i;
...
function getAuthTimeoutErrorPageState(options = {}) {
  ...
  const emailInUseBlocked = EMAIL_IN_USE_ERROR_CODE_PATTERN.test(text);
...
async function waitForAddEmailSubmitOutcome(timeout = 45000) {
  ...
  log(
    `步骤 8：添加邮箱提交后等待结果超时，最后页面状态：${getLoginAuthStateLabel(lastState)}，诊断快照：${JSON.stringify({
      state: lastState?.state || '',
      url: lastState?.url || location.href,
      displayedEmail: lastState?.displayedEmail || '',
      verificationVisible: Boolean(lastState?.verificationVisible),
      addEmailPage: Boolean(lastState?.addEmailPage),
      retryPage: Boolean(lastState?.retryPage),
      maxCheckAttemptsBlocked: Boolean(lastState?.maxCheckAttemptsBlocked),
      emailInUseBlocked: Boolean(lastState?.emailInUseBlocked),
      titleMatched: Boolean(lastState?.titleMatched),
      detailMatched: Boolean(lastState?.detailMatched),
    })}`,
    'warn'
  );
...
async function submitAddEmailAndContinue(payload = {}) {
  ...
  log(`步骤 8：添加邮箱提交结果：${JSON.stringify({
    success: Boolean(outcome?.success),
    verificationPage: Boolean(outcome?.verificationPage),
    directOAuthConsentPage: Boolean(outcome?.directOAuthConsentPage),
    retryPage: Boolean(outcome?.retryPage),
    emailInUse: Boolean(outcome?.emailInUse),
    maxCheckAttempts: Boolean(outcome?.maxCheckAttempts),
    displayedEmail: outcome?.displayedEmail || '',
    errorText: outcome?.errorText || '',
    url: outcome?.url || location.href,
  })}`);
  if (outcome.errorText && (SIGNUP_EMAIL_EXISTS_PATTERN.test(outcome.errorText) || EMAIL_IN_USE_ERROR_CODE_PATTERN.test(outcome.errorText))) {
    log(`步骤 8：检测到邮箱已被占用，字段报错：${outcome.errorText}`, 'warn');
    throw createStep8EmailInUseError();
  }
  if (outcome.errorText) {
    log(`步骤 8：添加邮箱字段报错：${outcome.errorText}`, 'warn');
    throw new Error(`添加邮箱失败：${outcome.errorText}`);
  }
  if (outcome.emailInUse) {
    log('步骤 8：添加邮箱后进入重试页，命中 email_in_use 标记。', 'warn');
    throw createStep8EmailInUseError();
  }
  if (outcome.maxCheckAttempts) {
    log('步骤 8：添加邮箱后进入重试页，命中 max_check_attempts 标记。', 'warn');
    throw createAuthMaxCheckAttemptsError();
  }
  if (outcome.retryPage) {
    log(`步骤 8：添加邮箱后进入认证重试页。URL: ${outcome.url}`, 'warn');

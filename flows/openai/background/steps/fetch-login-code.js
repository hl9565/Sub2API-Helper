(function attachBackgroundStep8(root, factory) {
  ...
  function createStep8Executor(deps = {}) {
    ...
    function isSub2ApiReauthMode(state = {}) {
      return Boolean(state?.sub2apiReauthMode);
    }

    function normalizeMailboxPollIntervalSeconds(state = {}) {
      const numeric = Number(state?.phoneCodePollIntervalSeconds);
    ...
    function normalizeMailboxPollMaxRounds(state = {}) {
      const numeric = Number(state?.phoneCodePollMaxRounds);
      if (!Number.isFinite(numeric)) {
        return 4;
      }
    ...
    function normalizeMailboxWaitSeconds(state = {}) {
      const numeric = Number(state?.phoneCodeWaitSeconds);
      if (!Number.isFinite(numeric)) {
        return 60;
      }
    ...
    function buildSub2ApiReauthMailPollOptions(state = {}) {
      if (!isSub2ApiReauthMode(state)) {
        return {};
      }
      const intervalSeconds = normalizeMailboxPollIntervalSeconds(state);
      const waitSeconds = normalizeMailboxWaitSeconds(state);
      const configuredRounds = normalizeMailboxPollMaxRounds(state);
      const maxAttempts = Math.max(configuredRounds, Math.ceil(waitSeconds / intervalSeconds));
      return {
    ...
    async function pollEmailVerificationCode(preparedState, pageState, visibleStep, runtime = {}) {
      ...
      await resolveVerificationStep(8, {
        ...
        ...buildSub2ApiReauthMailPollOptions(preparedState),

(function attachMultiPageOpenAiWorkflow(root, factory) {
  ...
  function isSub2ApiReauthModeEnabled(options = {}) {
    return Boolean(options?.sub2apiReauthMode);
  }

  function getSub2ApiReauthStepDefinitions() {
    const sourceByKey = new Map(
      (STEP_VARIANTS.normal || []).map((step) => [String(step.key || '').trim(), step])
    );
    const cloneStep = (key, overrides = {}) => ({
      ...(sourceByKey.get(key) || {}),
      ...overrides,
      key,
      flowId: 'openai',
    ...
    return [
      {
        id: 1,
        order: 10,
        key: 'select-sub2api-error-account',
        title: '选择 SUB2API 错误账号',
        sourceId: 'sub2api-panel',
        driverId: 'background/sub2api-api',
        command: 'select-sub2api-error-account',
        flowId: 'openai',
      },
      cloneStep('oauth-login', {
        id: 2,
        order: 20,
        title: '生成重授权链接并登录',
      }),
      cloneStep('fetch-login-code', {
        id: 3,
        order: 30,
        title: '获取登录验证码',
      }),
      cloneStep('confirm-oauth', {
        id: 4,
        order: 40,
        title: '自动确认 OAuth',
      }),
      cloneStep('platform-verify', {
        id: 5,
        order: 50,
        title: '写回 SUB2API 旧账号',
      }),
    ];
  }

  function resolveVariantKey(options = {}) {
    if (isSub2ApiReauthModeEnabled(options)) {
      return 'sub2apiReauth';
    }
  ...
  function getModeStepDefinitions(options = {}) {
    if (isSub2ApiReauthModeEnabled(options)) {
      return reindexModeStepDefinitions(getSub2ApiReauthStepDefinitions());
    }
  ...
  function getAllSteps() {
    const keyed = new Map();
    reindexModeStepDefinitions(getSub2ApiReauthStepDefinitions()).forEach((step) => {

...
test('auth page recovery detects email_already_in_use retry page state', () => {
  const state = {
    clickCount: 0,
    pageText: '糟糕，出错了！该邮箱地址已有相关联账户。请改为登录。错误代码：email_already_in_use',
    retryVisible: true,
  };
  const api = createRecoveryApi(state);
  ...
  assert.equal(Boolean(snapshot), true);
  assert.equal(snapshot.emailInUseBlocked, true);
});

test('auth page recovery throws email in use error without clicking retry', async () => {
  const state = {
    clickCount: 0,
    pageText: '糟糕，出错了！该邮箱地址已有相关联账户。请改为登录。错误代码：email_already_in_use',

...
test('OpenAI SUB2API reauth mode uses the error-account OAuth refresh chain', () => {
  const globalScope = {};
  const api = new Function('self', `${readStepDefinitionsBundle()}; return self.MultiPageStepDefinitions;`)(globalScope);
  const steps = api.getSteps({ sub2apiReauthMode: true });
  const nodes = api.getNodes({ sub2apiReauthMode: true });
  ...
  assert.deepStrictEqual(steps.map((step) => step.id), [1, 2, 3, 4, 5]);
  assert.equal(steps[0].title, '选择 SUB2API 错误账号');
  assert.equal(steps.find((step) => step.key === 'fetch-login-code')?.title, '获取登录验证码');
  assert.equal(steps.find((step) => step.key === 'confirm-oauth')?.title, '自动确认 OAuth');
  assert.equal(steps.at(-1).title, '写回 SUB2API 旧账号');
  assert.deepStrictEqual(nodes.map((node) => node.nodeId), steps.map((step) => step.key));
  assert.deepStrictEqual(nodes.find((node) => node.nodeId === 'select-sub2api-error-account')?.next, ['oauth-login']);
  assert.deepStrictEqual(nodes.find((node) => node.nodeId === 'fetch-login-code')?.next, ['confirm-oauth']);
  assert.deepStrictEqual(nodes.find((node) => node.nodeId === 'platform-verify')?.next, []);
});

...
test('step 4 applies Sub2API reauth mailbox polling settings', async () => {
  let capturedOptions = null;
  ...
  await executor.executeStep4({
    ...
    phoneCodePollIntervalSeconds: 6,
    phoneCodePollMaxRounds: 3,
  });

  assert.equal(capturedOptions.intervalMs, 6000);
  assert.equal(capturedOptions.maxAttempts, 12);
});

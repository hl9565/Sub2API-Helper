(function attachBackgroundMessageRouter(root, factory) {
  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handlePlatformVerifyStepData(payload) {
      ...
      if (latestState?.sub2apiReauthMode || payload?.sub2apiReauthCompleted) {
        const updates = {};
        if (payload.localhostUrl) {
          updates.localhostUrl = payload.localhostUrl;
        }
        if (payload.verifiedStatus) {
          updates.verifiedStatus = payload.verifiedStatus;
        }
        if (payload.sub2apiReauthCompleted !== undefined) {
          updates.sub2apiReauthCompleted = Boolean(payload.sub2apiReauthCompleted);
        }
        if (payload.sub2apiReauthAccountId !== undefined) {
          updates.sub2apiReauthAccountId = payload.sub2apiReauthAccountId || null;
        }
        if (payload.sub2apiReauthAccountName !== undefined) {
          updates.sub2apiReauthAccountName = payload.sub2apiReauthAccountName || '';
        }
        if (payload.sub2apiReauthSkipEmails !== undefined) {
          updates.sub2apiReauthSkipEmails = Array.isArray(payload.sub2apiReauthSkipEmails)
            ? payload.sub2apiReauthSkipEmails
            : String(payload.sub2apiReauthSkipEmails || '').split(/[\r\n,，;；\s]+/).map((item) => String(item || '').trim().toLowerCase()).filter(Boolean);
        }
        if (Object.keys(updates).length) {
          await setState(updates);
        }
        await addLog(payload.verifiedStatus || 'SUB2API 重新授权流程已完成。', 'ok', {
    ...
    async function handleStepData(step, payload) {
      if (step === 1) {
        ...
        if (payload.sub2apiReauthSkipEmails !== undefined) updates.sub2apiReauthSkipEmails = Array.isArray(payload.sub2apiReauthSkipEmails)
          ? payload.sub2apiReauthSkipEmails
          : String(payload.sub2apiReauthSkipEmails || '').split(/[\r\n,，;；\s]+/).map((item) => String(item || '').trim().toLowerCase()).filter(Boolean);

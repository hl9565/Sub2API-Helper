(function attachBackgroundFlowMailPolling(root, factory) {
  ...
  function getBrowserMailTransportErrorLimit(mail = {}) {
    return isIcloudMail(mail)
      ? ICLOUD_BROWSER_MAIL_TRANSPORT_ERROR_STREAK
      : DEFAULT_BROWSER_MAIL_TRANSPORT_ERROR_STREAK;
  }

  function isRetryableMailTransportError(error) {
    const message = String(error?.message || error || '');
    return /页面通信异常|页面未能重新就绪|内容脚本\s+\d+(?:\.\d+)?\s*秒内未响应|did not respond in \d+s|Receiving end does not exist|message channel is closed|port closed before a response was received|A listener indicated an asynchronous response|failed to fetch|networkerror|network error|fetch failed|load failed/i.test(message);
  }
  ...
  function createFlowMailPollingService(deps = {}) {
    ...
    async function pollThroughBrowserProvider(step, state, mail, pollPayload, options) {
      ...
      const maxTransportErrorStreak = Math.max(
        1,
        Math.floor(Number(options.maxTransportErrorStreak) || getBrowserMailTransportErrorLimit(mail))
      );
      let transportErrorStreak = 0;
      let lastTransportError = null;
      try {
        while (transportErrorStreak < maxTransportErrorStreak) {
          try {
            ...
            if (!isRetryableMailTransportError(error)) {
              throw error;
            }
            ...
            await log(
              `步骤 ${step}：${mail.label || '邮箱'}页面通信异常连续 ${transportErrorStreak}/${maxTransportErrorStreak} 次，正在重新尝试邮箱轮询。最后错误：${error.message}`,
              'warn',
              options.logOptions
            );
          }
        ...
        throw new Error(
          `步骤 ${step}：${mail.label || '邮箱'}页面通信异常连续 ${transportErrorStreak} 次，已判定邮箱轮询失败。最后错误：${lastTransportError?.message || '页面未能重新就绪'}`
        );
      } catch (error) {
        if (typeof isStopError === 'function' && isStopError(error)) {
          throw error;

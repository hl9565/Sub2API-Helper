(function attachBackgroundVerificationFlow(root, factory) {
  ...
  function createVerificationFlowHelpers(deps = {}) {
    ...
    async function pollFreshVerificationCode(step, state, mail, pollOverrides = {}) {
      ...
      let usedResendRequests = 0;
      const configuredIntervalMs = Math.max(1, Number(cleanPollOverrides.intervalMs) || 3000);
      const configuredMaxAttempts = Math.max(1, Number(cleanPollOverrides.maxAttempts) || 0);
      await addLog(
        `步骤 ${step}：邮箱验证码轮询设置：每 ${Math.ceil(configuredIntervalMs / 1000)} 秒轮询一次，每轮最多 ${configuredMaxAttempts} 次，最多自动重发 ${maxResendRequests} 次。`,
        'info'
      );

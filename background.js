...
const SUB2API_REAUTH_ACCOUNT_SELECT_TIMEOUT_MS = 60000;
...
const OPENAI_REAUTH_CLEAR_ORIGINS = [
...
const ICLOUD_LOCAL_DEDUP_MAX_ATTEMPTS = 3;
...
function getStepDefinitionsForState(state = {}) {
  ...
  if (rootScope.MultiPageStepDefinitions?.getSteps) {
    ...
    const definitions = rootScope.MultiPageStepDefinitions.getSteps({
      ...
      sub2apiReauthMode: Boolean(resolvedState?.sub2apiReauthMode),
...
function normalizePersistentSettingValue(key, value) {
  switch (key) {
    ...
    case 'sub2apiReauthSkipEmails':
      return normalizeEmailListSetting(value);
    case 'sub2apiReauthPageSize': {
...
function buildSettingsStatePatchFromFlatUpdates(updates = {}) {
  ...
  assignIfUpdated('phoneSmsProvider', ['services', 'phone', 'provider']);
  assignIfUpdated('phoneSmsProviderOrder', ['services', 'phone', 'providerOrder']);
  assignIfUpdated('verificationResendCount', ['services', 'phone', 'verificationResendCount']);
  assignIfUpdated('phoneVerificationReplacementLimit', ['services', 'phone', 'phoneVerificationReplacementLimit']);
  assignIfUpdated('phoneCodeWaitSeconds', ['services', 'phone', 'phoneCodeWaitSeconds']);
  assignIfUpdated('phoneCodeTimeoutWindows', ['services', 'phone', 'phoneCodeTimeoutWindows']);
  assignIfUpdated('phoneCodePollIntervalSeconds', ['services', 'phone', 'phoneCodePollIntervalSeconds']);
  assignIfUpdated('phoneCodePollMaxRounds', ['services', 'phone', 'phoneCodePollMaxRounds']);
  assignIfUpdated('phoneSmsReuseEnabled', ['services', 'phone', 'phoneSmsReuseEnabled']);
  assignIfUpdated('heroSmsReuseEnabled', ['services', 'phone', 'heroSms', 'reuseEnabled']);
  assignIfUpdated('freePhoneReuseEnabled', ['services', 'phone', 'freePhoneReuseEnabled']);
  assignIfUpdated('freePhoneReuseAutoEnabled', ['services', 'phone', 'freePhoneReuseAutoEnabled']);
  assignIfUpdated('phonePreferredActivation', ['services', 'phone', 'phonePreferredActivation']);
  assignIfUpdated('heroSmsApiKey', ['services', 'phone', 'heroSms', 'apiKey']);
  assignIfUpdated('heroSmsAcquirePriority', ['services', 'phone', 'heroSms', 'acquirePriority']);
  assignIfUpdated('heroSmsMinPrice', ['services', 'phone', 'heroSms', 'minPrice']);
  assignIfUpdated('heroSmsMaxPrice', ['services', 'phone', 'heroSms', 'maxPrice']);
  assignIfUpdated('heroSmsPreferredPrice', ['services', 'phone', 'heroSms', 'preferredPrice']);
  assignIfUpdated('heroSmsCountryId', ['services', 'phone', 'heroSms', 'countryId']);
  assignIfUpdated('heroSmsCountryLabel', ['services', 'phone', 'heroSms', 'countryLabel']);
  assignIfUpdated('heroSmsCountryFallback', ['services', 'phone', 'heroSms', 'countryFallback']);
  assignIfUpdated('fiveSimApiKey', ['services', 'phone', 'fiveSim', 'apiKey']);
  assignIfUpdated('fiveSimProduct', ['services', 'phone', 'fiveSim', 'product']);
  assignIfUpdated('fiveSimCountryId', ['services', 'phone', 'fiveSim', 'countryId']);
  assignIfUpdated('fiveSimCountryLabel', ['services', 'phone', 'fiveSim', 'countryLabel']);
  assignIfUpdated('fiveSimCountryFallback', ['services', 'phone', 'fiveSim', 'countryFallback']);
  assignIfUpdated('fiveSimCountryOrder', ['services', 'phone', 'fiveSim', 'countryOrder']);
  assignIfUpdated('fiveSimMinPrice', ['services', 'phone', 'fiveSim', 'minPrice']);
  assignIfUpdated('fiveSimMaxPrice', ['services', 'phone', 'fiveSim', 'maxPrice']);
  assignIfUpdated('fiveSimOperator', ['services', 'phone', 'fiveSim', 'operator']);
  assignIfUpdated('nexSmsApiKey', ['services', 'phone', 'nexSms', 'apiKey']);
  assignIfUpdated('nexSmsCountryOrder', ['services', 'phone', 'nexSms', 'countryOrder']);
  assignIfUpdated('nexSmsServiceCode', ['services', 'phone', 'nexSms', 'serviceCode']);
...
async function reserveGeneratedEmailLocally(email, options = {}) {
  const normalizedEmail = String(email || '').trim().toLowerCase();
  ...
  let response = null;
  try {
    response = await fetch(buildHotmailLocalEndpoint(helperBaseUrl, '/generated-email/check-and-reserve'), {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        provider,
        status: String(options.status || 'generated').trim().toLowerCase() || 'generated',
        flowId: String(options.flowId || state.flowId || state.activeFlowId || '').trim(),
        targetId: String(options.targetId || state.targetId || state.activeTargetId || '').trim(),
        source: String(options.source || '').trim(),
      }),
    });
  } catch (err) {
    return { ok: false, skipped: true, error: getErrorMessage(err) };
  }
  ...
  try {
    ...
    return { ok: false, skipped: true, error: `invalid json: ${getErrorMessage(err)}` };
  }

  if (!response.ok || payload?.ok === false) {
    return { ok: false, skipped: true, error: payload?.error || `HTTP ${response.status}` };
  }
  ...
  return {
    ...
    dbPath: String(payload?.dbPath || '').trim(),
  };
}

async function updateGeneratedEmailLocalStatus(email, status, options = {}) {
  const normalizedEmail = String(email || '').trim().toLowerCase();
  ...
  let response = null;
  try {
    response = await fetch(buildHotmailLocalEndpoint(helperBaseUrl, '/generated-email/update-status'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
...
async function fetchIcloudHideMyEmail(options = {}) {
  return withIcloudLoginHelp('获取 iCloud 隐私邮箱', async () => {
    ...
    try {
      for (let dedupAttempt = 1; dedupAttempt <= ICLOUD_LOCAL_DEDUP_MAX_ATTEMPTS; dedupAttempt += 1) {
        ...
        const reserveData = {
          ...
          note: 'Generated through Sub2API Helper',
        };
        ...
        const localReserve = await reserveGeneratedEmailLocally(alias, {
          provider: 'icloud',
          source: options?.source || 'generated:icloud',
          state: options?.state || null,
          status: 'generated',
        });
        if (localReserve.ok && localReserve.duplicate) {
          const dbPathSuffix = localReserve.dbPath ? `（本地库：${localReserve.dbPath}）` : '';
          await addLog(`iCloud：本地库检测到别名 ${alias} 已存在，正在重新生成${dbPathSuffix}`, 'warn');
          if (dedupAttempt >= ICLOUD_LOCAL_DEDUP_MAX_ATTEMPTS) {
            throw new Error(`iCloud 隐私邮箱本地去重失败：连续 ${ICLOUD_LOCAL_DEDUP_MAX_ATTEMPTS} 次命中重复别名 ${alias}。`);
          }
          continue;
        }
        if (!localReserve.ok && localReserve.error) {
          await addLog(`iCloud：本地去重不可用，已跳过本次去重（${localReserve.error}）`, 'warn');
        }
      ...
      throw new Error(`iCloud 隐私邮箱本地去重失败：超过最大重试次数 ${ICLOUD_LOCAL_DEDUP_MAX_ATTEMPTS}。`);
    } catch (err) {
...
async function finalizeIcloudAliasAfterSuccessfulFlow(state) {
  ...
  const localStatusResult = await updateGeneratedEmailLocalStatus(email, 'used', { state });
  if (!localStatusResult.ok && localStatusResult.error) {
    await addLog(`iCloud：本地库写入 used 状态失败（${localStatusResult.error}）`, 'warn');
  }
  ...
  if (step <= 1) {
    return {
      ...
      sub2apiGroupId: null,
              ...
              sub2apiProxyId: null,
              sub2apiReauthAccountId: null,
              sub2apiReauthAccountName: '',
              sub2apiReauthAccountStatus: '',
              sub2apiReauthAccountProxyId: null,
              sub2apiReauthCompleted: false,
...
async function handleStepData(step, payload) {
  ...
  function shouldSkipPendingAuthChainNodesOnDirectOauth(stepKey, stepPayload = {}) {
    if (!Boolean(stepPayload?.directOAuthConsentPage)) {
      return false;
    }
    return stepKey === 'fetch-login-code'
  ...
  async function skipPendingAuthChainNodesBeforeConfirmOauth(startStep) {
    const latestState = await getState();
    const startNodeId = getNodeIdByStepForState(startStep, latestState);
    const confirmOauthStep = Number(getStepIdByKeyForState('confirm-oauth', latestState)) || 0;
    const confirmOauthNodeId = confirmOauthStep > 0
      ? getNodeIdByStepForState(confirmOauthStep, latestState)
      : '';
    const nodeIds = getNodeIdsForState(latestState);
    const startIndex = nodeIds.indexOf(startNodeId);
    const confirmIndex = nodeIds.indexOf(confirmOauthNodeId);
    if (startIndex < 0 || confirmIndex < 0 || startIndex >= confirmIndex) {
      return [];
    }

    const skippedNodeIds = [];
    for (let index = startIndex + 1; index < confirmIndex; index += 1) {
      const candidateNodeId = String(nodeIds[index] || '').trim();
      if (!candidateNodeId || !isAuthChainNode(candidateNodeId)) {
        continue;
      }
      const currentStatus = String(latestState.nodeStatuses?.[candidateNodeId] || 'pending').trim() || 'pending';
      if (currentStatus === 'running' || currentStatus === 'completed' || currentStatus === 'manual_completed' || currentStatus === 'skipped') {
        continue;
      }
      await setNodeStatus(candidateNodeId, 'skipped');
      skippedNodeIds.push(candidateNodeId);
    }
  ...
  switch (step) {
    case 1: {
      ...
      if (payload.sub2apiReauthAccountName !== undefined) updates.sub2apiReauthAccountName = payload.sub2apiReauthAccountName || '';
      if (payload.sub2apiReauthAccountEmail !== undefined) updates.sub2apiReauthAccountEmail = payload.sub2apiReauthAccountEmail || '';
      if (payload.sub2apiReauthAccountNotes !== undefined) updates.sub2apiReauthAccountNotes = payload.sub2apiReauthAccountNotes || '';
      ...
      if (payload.accountIdentifier !== undefined) updates.accountIdentifier = String(payload.accountIdentifier || '').trim();
      if (payload.sub2apiReauthAccountStatus !== undefined) updates.sub2apiReauthAccountStatus = payload.sub2apiReauthAccountStatus || '';
      if (payload.sub2apiReauthAccountProxyId !== undefined) updates.sub2apiReauthAccountProxyId = payload.sub2apiReauthAccountProxyId || null;
      if (payload.sub2apiReauthQueueTotal !== undefined) updates.sub2apiReauthQueueTotal = Math.max(0, Number(payload.sub2apiReauthQueueTotal) || 0);
      if (payload.sub2apiReauthQueuePage !== undefined) updates.sub2apiReauthQueuePage = Math.max(1, Number(payload.sub2apiReauthQueuePage) || 1);
      if (payload.sub2apiReauthQueuePageSize !== undefined) updates.sub2apiReauthQueuePageSize = Math.max(1, Number(payload.sub2apiReauthQueuePageSize) || 1);
      if (payload.sub2apiReauthCompleted !== undefined) updates.sub2apiReauthCompleted = Boolean(payload.sub2apiReauthCompleted);
  ...
  const latestState = await getState();
  const stepKey = getStepExecutionKeyForState(step, latestState);
  if (shouldSkipPendingAuthChainNodesOnDirectOauth(stepKey, payload)) {
    const skippedNodeIds = await skipPendingAuthChainNodesBeforeConfirmOauth(step);
    if (skippedNodeIds.length) {
      await addLog(`步骤 ${step}：认证页已直接进入 OAuth 授权页，已跳过后续前置节点 ${skippedNodeIds.join('、')}。`, 'warn');
    }
  }
}
...
async function appendAndBroadcastAccountRunRecord(status, stateOverride = null, reason = '') {
  ...
  const currentEmail = String(record.email || state.email || '').trim().toLowerCase();
  if (currentEmail && normalizeEmailGenerator(state.emailGenerator) === 'icloud') {
    const localStatus = resolvedStatus === 'success'
      ? 'used'
      : (resolvedStatus === 'failed' ? 'failed' : (resolvedStatus === 'stopped' ? 'stopped' : ''));
    if (localStatus) {
      const localStatusResult = await updateGeneratedEmailLocalStatus(currentEmail, localStatus, { state });
      if (!localStatusResult.ok && localStatusResult.error) {
        await addLog(`iCloud：本地库写入 ${localStatus} 状态失败（${localStatusResult.error}）`, 'warn');
      }
    }
  }
...
const autoRunController = self.MultiPageBackgroundAutoRunController?.createAutoRunController({
  ...
  onAutoRunRoundFailure: async (payload = {}) => {
    const latestState = await getState();
...
async function ensureAutoEmailReady(targetRun, totalRuns, attemptRuns) {
  const currentState = await getState();
  if (currentState?.sub2apiReauthMode) {
    const accountEmail = String(currentState.sub2apiReauthAccountEmail || currentState.email || '').trim().toLowerCase();
    if (!accountEmail) {
      throw new Error('SUB2API 重新授权：当前错误账号没有邮箱，无法登录 OpenAI。请检查 SUB2API 账号信息。');
    }
    await setEmailStateSilently(accountEmail, { source: 'sub2api_reauth_account' });
    await addLog(`SUB2API 重新授权：使用旧账号邮箱 ${accountEmail} 登录 OpenAI，不生成新邮箱。`, 'info');
    return accountEmail;
  }
...
async function runAutoSequenceFromNodeGraph(startNodeId, context = {}) {
  ...
  if (activeFlowId !== defaultActiveFlowId || Boolean(initialFlowState?.sub2apiReauthMode)) {
...
let sub2ApiReauthApi = null;

function getSub2ApiReauthApi() {
  if (sub2ApiReauthApi) {
    return sub2ApiReauthApi;
  }
  const factory = self.MultiPageBackgroundSub2ApiApi?.createSub2ApiApi;
  if (typeof factory !== 'function') {
    throw new Error('SUB2API 直连接口模块未加载，无法选择重新授权账号。');
  }
  sub2ApiReauthApi = factory({
    addLog,
    normalizeSub2ApiUrl,
    DEFAULT_SUB2API_GROUP_NAME,
  });
...
async function clearOpenAiReauthBrowserState(options = {}) {
  const logLabel = String(options.logLabel || 'SUB2API 重新授权').trim();
  const dataTypes = {
    cache: true,
    cacheStorage: true,
    cookies: true,
  ...
  await addLog(`${logLabel}：正在清理 OpenAI/Auth/ChatGPT Cookie 和站点数据，避免复用旧登录态...`, 'info', options.logOptions || {});
  try {
    if (chrome?.browsingData?.remove) {
      await chrome.browsingData.remove({ origins: OPENAI_REAUTH_CLEAR_ORIGINS }, dataTypes);
    } else if (chrome?.browsingData?.removeCookies) {
      await chrome.browsingData.removeCookies({ origins: OPENAI_REAUTH_CLEAR_ORIGINS });
    }
    await addLog(`${logLabel}：OpenAI 相关站点数据已清理。`, 'ok', options.logOptions || {});
  } catch (error) {
    await addLog(`${logLabel}：清理 OpenAI 站点数据失败，继续流程但可能复用旧登录态：${getErrorMessage(error)}`, 'warn', options.logOptions || {});
  }
}

function isSub2ApiReauthCodeFailure(state = {}, reason = '') {
  const nodeId = String(state?.currentNodeId || '').trim();
  if (['fetch-login-code', 'fetch-bind-email-code', 'fetch-bound-email-login-code'].includes(nodeId)) {
    return true;
  }
  return /验证码|接码|轮询|未在.*邮箱|暂未.*验证码|无法获取新的(?:登录|绑定邮箱)?验证码|code/i.test(String(reason || ''));
}

function resolveSub2ApiReauthRetryAt(state = {}) {
  const waitSeconds = normalizePhoneCodeWaitSeconds(state?.phoneCodeWaitSeconds, DEFAULT_PHONE_CODE_WAIT_SECONDS);
  const timeoutWindows = normalizePhoneCodeTimeoutWindows(state?.phoneCodeTimeoutWindows, DEFAULT_PHONE_CODE_TIMEOUT_WINDOWS);
  const retryDelayMs = Math.max(60, waitSeconds * timeoutWindows) * 1000;
  return new Date(Date.now() + retryDelayMs).toISOString();
}

async function updateSub2ApiReauthFailureNotes(state = {}, reason = '', options = {}) {
  if (!state?.sub2apiReauthMode || !state?.sub2apiReauthAccountId || state?.sub2apiReauthCompleted) {
    return null;
  }
  const api = getSub2ApiReauthApi();
  const failureKind = isSub2ApiReauthCodeFailure(state, reason) ? '接码失败' : '重新授权失败';
  const retryAt = resolveSub2ApiReauthRetryAt(state);
  const result = await api.updateOpenAiReauthFailureNotes(state, {
    logLabel: 'SUB2API 重新授权失败',
    logOptions: { nodeId: state.currentNodeId || '' },
    reason,
    retryAt,
...
async function executeSelectSub2ApiErrorAccount(state = {}) {
  const visibleStep = Math.floor(Number(state?.visibleStep) || 0) || 1;
  const sub2apiUrl = normalizeSub2ApiUrl(state.sub2apiUrl);
  if (!sub2apiUrl) {
    throw new Error('尚未配置 SUB2API 地址，请先在侧边栏填写。');
  }
  ...
  const api = getSub2ApiReauthApi();
  const result = await api.selectOpenAiOAuthErrorAccountForReauth({
    ...state,
    sub2apiUrl,
  }, {
    logLabel: `步骤 ${visibleStep}`,
    logOptions: { step: visibleStep, stepKey: 'select-sub2api-error-account' },
    timeoutMs: SUB2API_REAUTH_ACCOUNT_SELECT_TIMEOUT_MS,
    emailSuffix: state.sub2apiReauthEmailSuffix || '',
    skipEmails: state.sub2apiReauthSkipEmails || [],
    pageSize: state.sub2apiReauthPageSize || 50,
  });
  await clearOpenAiReauthBrowserState({
    logLabel: `步骤 ${visibleStep}`,
    logOptions: { step: visibleStep, stepKey: 'select-sub2api-error-account' },
  });
  await completeNodeFromBackground(state?.nodeId || 'select-sub2api-error-account', result);
}
...
const stepExecutorsByKey = {
  'select-sub2api-error-account': (state) => executeSelectSub2ApiErrorAccount(state),

...
GENERATED_EMAIL_DB_PATH = os.path.join(BASE_DIR, "data", "generated-emails.db")
ACCOUNT_RECORDS_LOCK = threading.Lock()
GENERATED_EMAIL_DB_LOCK = threading.Lock()
...
def ensure_generated_email_db():
    os.makedirs(os.path.dirname(GENERATED_EMAIL_DB_PATH), exist_ok=True)
    with GENERATED_EMAIL_DB_LOCK:
        conn = sqlite3.connect(GENERATED_EMAIL_DB_PATH)
        try:
            conn.execute(
                """
...
def normalize_generated_email_payload(payload):
    if not isinstance(payload, dict):
        raise RuntimeError("Invalid generated email payload")

    email_addr = str(payload.get("email") or "").strip().lower()
    provider = str(payload.get("provider") or "").strip().lower()
    if not email_addr or "@" not in email_addr:
        raise RuntimeError("Missing generated email")
    if not provider:
        raise RuntimeError("Missing generated email provider")
    ...
    return {
        ...
        "created_at": str(payload.get("createdAt") or "").strip() or now,
        "last_seen_at": now,
    }


def reserve_generated_email(payload):
    ensure_generated_email_db()
    normalized = normalize_generated_email_payload(payload)
    ...
    with GENERATED_EMAIL_DB_LOCK:
        ...
        try:
            existing = conn.execute(
                """
                SELECT email, provider, status, flow_id, target_id, source, created_at, last_seen_at, used_at
            ...
            if existing:
                conn.execute(
                    "UPDATE generated_emails SET last_seen_at = ? WHERE email = ?",
                    (normalized["last_seen_at"], normalized["email"]),
                )
                conn.commit()
                return {
                    "ok": True,
                    ...
                    "dbPath": GENERATED_EMAIL_DB_PATH,
                }

            conn.execute(
                """
                INSERT INTO generated_emails (
                    email, provider, status, flow_id, target_id, source, created_at, last_seen_at, used_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, '')
                """,
                (
                    normalized["email"],
            ...
            return {
                "ok": True,
                "duplicate": False,
                "record": dict(created) if created else {
...
def update_generated_email_status(payload):
    ensure_generated_email_db()
    if not isinstance(payload, dict):
        raise RuntimeError("Invalid generated email status payload")
    ...
    if not status:
        raise RuntimeError("Missing generated email status")
    ...
    with GENERATED_EMAIL_DB_LOCK:
        ...
        try:
            ...
            used_at = now if status == "used" else str(existing["used_at"] or "").strip()
            conn.execute(
                """
                UPDATE generated_emails
                SET status = ?, last_seen_at = ?, used_at = ?
                WHERE email = ?
                """,
                (status, now, used_at, email_addr),
            )
            conn.commit()
            ...
            return {
                "ok": True,
                "updated": True,
                "record": dict(updated) if updated else None,
                "dbPath": GENERATED_EMAIL_DB_PATH,
            }
        finally:
            conn.close()
...
class HotmailHelperHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/generated-email/check-and-reserve":
                result = reserve_generated_email(payload)
                json_response(self, 200, result)
                return

            if self.path == "/generated-email/update-status":
                result = update_generated_email_status(payload)
                json_response(self, 200, result)
                return
...
def main():
    ...
    print(f"Generated email DB: {GENERATED_EMAIL_DB_PATH}", flush=True)
    print("Generated email APIs: POST /generated-email/check-and-reserve, POST /generated-email/update-status", flush=True)
    try:

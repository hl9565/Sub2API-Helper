...
const inputSub2ApiReauthSkipEmails = document.getElementById('input-sub2api-reauth-skip-emails');
...
function isSub2ApiReauthModeSelected() {
  const input = typeof inputSub2ApiReauthMode !== 'undefined' ? inputSub2ApiReauthMode : null;
  return Boolean(input?.checked || (typeof latestState !== 'undefined' && latestState?.sub2apiReauthMode));
}

function withSub2ApiReauthModeOption(options = {}, enabled = false) {
  return enabled ? { ...options, sub2apiReauthMode: true } : options;
}
...
function getStepDefinitionsForMode(plusModeEnabled = false, options = {}) {
  ...
  const sub2apiReauthMode = typeof options === 'string'
    ? Boolean(typeof latestState !== 'undefined' ? latestState?.sub2apiReauthMode : false)
    : Boolean(options.sub2apiReauthMode ?? (typeof latestState !== 'undefined' ? latestState?.sub2apiReauthMode : false));
  ...
  if (sub2apiReauthMode) {
    stepOptions.sub2apiReauthMode = true;
  }
...
function getWorkflowNodesForMode(plusModeEnabled = false, options = {}) {
  ...
  if (sub2apiReauthMode) {
    nodeOptions.sub2apiReauthMode = true;
  }
...
function rebuildStepDefinitionState(plusModeEnabled = false, options = {}) {
  ...
  const sub2apiReauthMode = Boolean(
    options.sub2apiReauthMode
    ?? (typeof inputSub2ApiReauthMode !== 'undefined' && inputSub2ApiReauthMode
      ? inputSub2ApiReauthMode.checked
      : (typeof latestState !== 'undefined' ? latestState?.sub2apiReauthMode : false))
  );
  ...
  if (sub2apiReauthMode) {
    stepDefinitionOptions.sub2apiReauthMode = true;
  }
  stepDefinitions = getStepDefinitionsForMode(currentPlusModeEnabled, stepDefinitionOptions);
  const nextWorkflowNodes = typeof getWorkflowNodesForMode === 'function'
    ? getWorkflowNodesForMode(currentPlusModeEnabled, stepDefinitionOptions)
...
const SUB2API_HELPER_REAUTH_ONLY = true;
...
function lockSub2ApiHelperUi() {
  if (!SUB2API_HELPER_REAUTH_ONLY) {
    return;
  }
  if (selectFlow) {
    selectFlow.value = 'openai';
  }
  if (selectPanelMode) {
    selectPanelMode.value = 'sub2api';
  }
  if (inputSub2ApiReauthMode) {
    inputSub2ApiReauthMode.checked = true;
  }
  ...
  [
    ...
    document.getElementById('row-source-selector'),
    rowPlusMode,
    ...
    document.getElementById('ip-proxy-section'),
  ].forEach((element) => {
    if (element) {
  ...
  [
    ...
    rowCustomPassword,
    document.getElementById('row-mail-provider'),
    document.getElementById('phone-verification-section'),
    rowPhoneVerificationFold,
    document.getElementById('row-auto-run-controls'),
...
function collectSettingsPayload() {
  ...
  const selectedEmailGenerator = typeof getSelectedEmailGenerator === 'function'
    ? getSelectedEmailGenerator()
    : String(selectEmailGenerator?.value || latestState?.emailGenerator || 'duck').trim().toLowerCase();
  ...
  const sub2apiReauthModeEnabled = Boolean(
    (typeof inputSub2ApiReauthMode !== 'undefined' && inputSub2ApiReauthMode?.checked)
    || (typeof latestState !== 'undefined' && latestState?.sub2apiReauthMode)
  );
  ...
  const effectivePlusModeEnabled = sub2apiReauthModeEnabled
    ? false
    : capabilityState
  ...
  return {
    ...
    sub2apiReauthEmailSuffix: String(
      typeof inputSub2ApiReauthEmailSuffix !== 'undefined' && inputSub2ApiReauthEmailSuffix
        ...
        : latestState?.sub2apiReauthEmailSuffix || ''
    ).trim().toLowerCase(),
    sub2apiReauthSkipEmails: String(
      typeof inputSub2ApiReauthSkipEmails !== 'undefined' && inputSub2ApiReauthSkipEmails
        ? inputSub2ApiReauthSkipEmails.value
        : latestState?.sub2apiReauthSkipEmails || ''
    ),
    sub2apiReauthPageSize: latestState?.sub2apiReauthPageSize || 50,
    ...
    emailGenerator: selectedEmailGenerator,
...
function updatePhoneVerificationSettingsUI() {
  const reauthOnlyMode = typeof SUB2API_HELPER_REAUTH_ONLY !== 'undefined' && SUB2API_HELPER_REAUTH_ONLY;
  if (reauthOnlyMode) {
    const phoneVerificationSection = document.getElementById('phone-verification-section');
    if (phoneVerificationSection) {
      phoneVerificationSection.style.display = '';
      const sectionLabel = phoneVerificationSection.querySelector?.('.section-label');
      if (sectionLabel) {
        sectionLabel.textContent = '邮箱接码设置';
      }
      const sectionDescription = phoneVerificationSection.querySelector?.('.section-mini-copy .data-value');
      if (sectionDescription) {
        sectionDescription.textContent = '邮箱验证码轮询与重发策略';
      }
    }
    if (rowPhoneVerificationEnabled) {
      rowPhoneVerificationEnabled.style.display = 'none';
    }
    if (rowPhoneVerificationFold) {
      rowPhoneVerificationFold.style.display = '';
    }
    [
      rowSignupMethod,
      rowPhoneSignupReloginAfterBindEmail,
...
function syncStepDefinitionsForMode(plusModeEnabled = false, plusPaymentMethodOrOptions = {}, maybeOptions = {}) {
  ...
  const nextSub2ApiReauthMode = Boolean(
    options.sub2apiReauthMode
      ?? (typeof inputSub2ApiReauthMode !== 'undefined' && inputSub2ApiReauthMode
        ? inputSub2ApiReauthMode.checked
        : (typeof latestState !== 'undefined' ? latestState?.sub2apiReauthMode : false))
  ...
  if (nextSub2ApiReauthMode) {
    paymentTitleOptions.sub2apiReauthMode = true;
  }
  const nextPaymentTitle = rootScope.MultiPageStepDefinitions?.getPlusPaymentStepTitle?.(paymentTitleOptions);
  ...
  const shouldRender = Boolean(options.render)
    ...
    || nextSub2ApiReauthMode !== Boolean(typeof latestState !== 'undefined' ? latestState?.sub2apiReauthMode : false)
  ...
  if (nextSub2ApiReauthMode) {
    rebuildOptions.sub2apiReauthMode = true;
  }
  rebuildStepDefinitionState(nextPlusModeEnabled, rebuildOptions);
  renderStepsList();
}
...
function applySettingsState(state) {
  ...
  if (typeof inputSub2ApiReauthSkipEmails !== 'undefined' && inputSub2ApiReauthSkipEmails) {
    inputSub2ApiReauthSkipEmails.value = Array.isArray(state?.sub2apiReauthSkipEmails)
      ? state.sub2apiReauthSkipEmails.join('\n')
      : String(state?.sub2apiReauthSkipEmails || '');
  }
  ...
  const yydsMailProvider = typeof YYDS_MAIL_PROVIDER === 'string'
    ? YYDS_MAIL_PROVIDER
    : 'yyds-mail';
  const restoredMailProvider = state?.mailProvider || ICLOUD_PROVIDER;
  ...
  updatePanelModeUI();
  updateMailProviderUI();
  if (typeof lockSub2ApiHelperUi === 'function') {
    lockSub2ApiHelperUi();
  }
...
function getRepositoryHomeUrl() {
  return 'https://github.com/hl9565/Sub2API-Helper';
}
...
function renderUpdateReleaseList(releases = []) {
  ...
  releases.forEach((release) => {
    ...
    version.textContent = release.displayVersion || `Sub2API Helper ${release.version}`;
...
function renderReleaseSnapshot(snapshot) {
  ...
  switch (snapshot?.status) {
    ...
    case 'ignored': {
      extensionUpdateStatus.textContent = localVersionText || 'Sub2API Helper 0.0.1';
    ...
    default: {
      ...
      extensionVersionMeta.textContent = snapshot?.errorMessage || '更新检查失败';
...
async function initializeReleaseInfo() {
  ...
  const localVersion = sidepanelUpdateService?.getLocalVersionLabel?.(chrome.runtime.getManifest())
    ...
    || (chrome.runtime.getManifest()?.version ? `Sub2API Helper ${chrome.runtime.getManifest().version}` : '');
  extensionUpdateStatus.textContent = localVersion || 'Sub2API Helper 0.0.1';
...
function updateMailProviderUI() {
  ...
  const showIcloudHostOnlyForReauth = reauthOnlyMode && useIcloudProvider;
  ...
  if (rowEmailGenerator) {
    rowEmailGenerator.style.display = reauthOnlyMode ? 'none' : (useEmailGenerator ? '' : 'none');
  }
  ...
  if (icloudSection) {
    const showIcloudSection = showIcloudHostOnlyForReauth || (!reauthOnlyMode && ((useEmailGenerator && useIcloud) || useIcloudProvider));
    icloudSection.style.display = showIcloudSection ? '' : 'none';
    const icloudSectionLabel = icloudSection.querySelector?.('.section-label');
    if (icloudSectionLabel) {
      icloudSectionLabel.textContent = showIcloudHostOnlyForReauth ? 'iCloud 邮箱' : 'iCloud 隐私邮箱';
    }
    const icloudSectionActions = icloudSection.querySelector?.('.section-mini-actions');
    if (icloudSectionActions) {
      icloudSectionActions.style.display = showIcloudHostOnlyForReauth ? 'none' : '';
    }
    const icloudFetchModeRow = typeof selectIcloudFetchMode !== 'undefined'
      ? selectIcloudFetchMode?.closest?.('.data-row')
      : null;
    if (icloudFetchModeRow) {
      icloudFetchModeRow.style.display = showIcloudHostOnlyForReauth ? 'none' : '';
    }
    const icloudAutoDeleteRow = typeof checkboxAutoDeleteIcloud !== 'undefined'
      ? checkboxAutoDeleteIcloud?.closest?.('.data-row')
      : null;
    if (icloudAutoDeleteRow) {
      icloudAutoDeleteRow.style.display = showIcloudHostOnlyForReauth ? 'none' : '';
    }
    const icloudCard = icloudSection.querySelector?.('.icloud-card');
    if (icloudCard) {
      icloudCard.style.display = showIcloudHostOnlyForReauth ? 'none' : '';
    }
    if (showIcloudSection && !showIcloudHostOnlyForReauth) {
  ...
  if (typeof rowIcloudTargetMailboxType !== 'undefined' && rowIcloudTargetMailboxType) {
    rowIcloudTargetMailboxType.style.display = showIcloudTargetMailboxType && !showIcloudHostOnlyForReauth ? '' : 'none';
  }
  if (typeof rowIcloudForwardMailProvider !== 'undefined' && rowIcloudForwardMailProvider) {
    rowIcloudForwardMailProvider.style.display = showIcloudForwardMailProvider && !showIcloudHostOnlyForReauth ? '' : 'none';
  }
  ...
  btnFetchEmail.hidden = reauthOnlyMode || useHotmail || useLuckmail || useCustomEmail || useCustomEmailPool;
  inputEmail.readOnly = reauthOnlyMode || useHotmail || useLuckmail;
  if (reauthOnlyMode) {
    inputEmail.placeholder = '自动读取 SUB2API 待授权账号邮箱';
    if (autoHintText) {
      autoHintText.textContent = '会自动读取 SUB2API 待授权账号邮箱，收码邮箱只用于接收验证码。';
    }
...
function updatePanelModeUI() {
  if (typeof SUB2API_HELPER_REAUTH_ONLY !== 'undefined' && SUB2API_HELPER_REAUTH_ONLY) {
    if (selectFlow) {
      selectFlow.value = 'openai';
    }
    if (selectPanelMode) {
...
async function startAutoRunFromCurrentSettings() {
  ...
  const reauthModeEnabled = reauthOnlyMode || Boolean(
    (typeof inputSub2ApiReauthMode !== 'undefined' && inputSub2ApiReauthMode?.checked)
    || (typeof latestState !== 'undefined' && latestState?.sub2apiReauthMode)
  );
  if (!reauthOnlyMode && reauthModeEnabled && getSelectedTargetId(getSelectedFlowId(latestState)) !== 'sub2api') {
    throw new Error('SUB2API 重新授权模式需要先选择来源为 SUB2API。');
  }
  if (reauthModeEnabled) {
    if (!String(inputSub2ApiUrl?.value || latestState?.sub2apiUrl || '').trim()) {
      throw new Error('请先填写 SUB2API 地址。');
    }
    if (!String(inputSub2ApiEmail?.value || latestState?.sub2apiEmail || '').trim()) {
      throw new Error('请先填写 SUB2API 登录账号。');
    }
    if (!String(inputSub2ApiPassword?.value || latestState?.sub2apiPassword || '').trim()) {
  ...
  const autoRunStartValidation = (() => {
    ...
    const validationState = {
      ...
      plusModeEnabled: typeof inputPlusModeEnabled !== 'undefined' && inputPlusModeEnabled
        ? Boolean(inputPlusModeEnabled.checked)
        : Boolean(latestState?.plusModeEnabled),
      sub2apiReauthMode: reauthModeEnabled,
  ...
  const activeFlowId = reauthOnlyMode
    ? 'openai'
    : typeof getSelectedFlowId === 'function'
    ? getSelectedFlowId(latestState)
    : (String(latestState?.activeFlowId || latestState?.flowId || DEFAULT_ACTIVE_FLOW_ID).trim().toLowerCase() || DEFAULT_ACTIVE_FLOW_ID);
  const targetId = reauthOnlyMode
    ? 'sub2api'
    : typeof getSelectedTargetId === 'function'
    ? getSelectedTargetId(activeFlowId)
...
btnAutoRun.addEventListener('click', async () => {
  try {
    ...
    clearPendingAutoRunStartRunCount();
    setDefaultAutoRunButton();
    inputRunCount.disabled = Boolean(
      (typeof inputSub2ApiReauthMode !== 'undefined' && inputSub2ApiReauthMode?.checked)
      || (typeof latestState !== 'undefined' && latestState?.sub2apiReauthMode)
    )
      ? false
...
inputSub2ApiReauthMode?.addEventListener('change', () => {
  if (inputSub2ApiReauthMode.checked) {
    selectPanelMode.value = 'sub2api';
    if (inputPlusModeEnabled) {
      inputPlusModeEnabled.checked = false;
    }
    showToast('已切换为 SUB2API 错误账号重新授权流程', 'info', 1800);
  }
  updatePanelModeUI();
  updatePlusModeUI();
  syncLatestState({
    targetId: 'sub2api',
    plusModeEnabled: false,
    sub2apiReauthMode: Boolean(inputSub2ApiReauthMode.checked),
  });
  rebuildStepDefinitionState(false, {
    activeFlowId: getSelectedFlowId(latestState),
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      const shouldDeferDataUpdatedUiApply = isEditableElementInSettingsCard(activeSettingsEditor);
      if (shouldDeferDataUpdatedUiApply) {
        // Avoid overwriting the focused editor while the user is still editing
        // settings inputs; the latest state is already synced above.
      ...
      if (
        message.payload.sub2apiReauthMode !== undefined
        && typeof inputSub2ApiReauthMode !== 'undefined'
        && inputSub2ApiReauthMode
      ) {
      ...
      if (
        ...
        && typeof inputSub2ApiReauthSkipEmails !== 'undefined'
        && inputSub2ApiReauthSkipEmails
      ) {
        inputSub2ApiReauthSkipEmails.value = Array.isArray(message.payload.sub2apiReauthSkipEmails)
          ? message.payload.sub2apiReauthSkipEmails.join('\n')
          : String(message.payload.sub2apiReauthSkipEmails || '');
      }
      if (
...
extensionUpdateStatus?.addEventListener('keydown', (event) => {
